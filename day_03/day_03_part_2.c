...
int consume_dont(const char *input, int length, int *offset);

int consume_do(const char *input, int length, int *offset);
...
int main(void) {
    ...
    for (int i = 0; i < contents_length;) {
        int new_offset = i;
        if (consume_do(contents, contents_length, &new_offset)) {
            enabled = 1;
            i = new_offset;
            continue;
        } else {
        ...
        if (consume_dont(contents, contents_length, &new_offset)) {
            enabled = 0;
            i = new_offset;
            continue;
        } else {
...
int consume_dont(const char *input, int length, int *offset) {
    if (!consume_char(input, length, offset, 'd')) return 0;
    if (!consume_char(input, length, offset, 'o')) return 0;
    if (!consume_char(input, length, offset, 'n')) return 0;
    if (!consume_char(input, length, offset, '\'')) return 0;
    if (!consume_char(input, length, offset, 't')) return 0;
    if (!consume_char(input, length, offset, '(')) return 0;
    if (!consume_char(input, length, offset, ')')) return 0;
    return 1;
}
